...
(function($) {
	...
	$.fn.chameleon = function (params){

		//"use-strict";

		var options = ('options' in params) ? params['options'] : {};
		var positions = ('positions' in params) ? params['positions'] : {};

		// If the options are a command and this command exists...
		if (typeof params === 'string' && options in $.fn.chameleon.externalFunctions)
			return $.fn.chameleon.externalFunctions[params]();
		...
		// Else, maybe they are a dict initializing the viewer...
			...
			if (!($(this).selector in viewers) && $(this).length > 0){
				...
				// Add the cursor pixels positions and dragging state for the moving image event
		...
		function initViewer (selector){
			...
			_showImage(selector, viewers[selector].options.currentImage);

			// Set the viewer title
			$(selector + ' .chameleon-title').text(viewers[selector].options.title);

			// Set the images total number
			_setImageTotalNumber(selector, viewers[selector].options.images.length);
		 ...
		 * Params:
		 *     - selector    {string}    the viewer element
		 *     - position    {integer}   the position of the image to be displayed
		 */
		function _showImage (selector, position){
			...
			if (options.images.length > position && position >= 0){
				...
				// ... show the image properties correctly (if they are enabled),...
				if ('url' in image){
					$(selector).css('background-image', 'url("' + image.url + '")');
					// ... update the image size and center it...
					_updateBackgroundImageSizeOnLoad(selector, image.url);
				}
		 ...
		 */
		function _updateBackgroundImageSizeOnLoad (selector, imageURL){
		 ...
		 */
		function _updateBackgroundImagePosition (selector, zoom=null){
		 ...
		 */
		function _setImageTotalNumber (selector, number){
		...
		function _changeImage (selector, direction){
			...
			_showImage(selector, _calcImagePosition(selector, direction));
		 ...
		 * Return: the image position to show.
		 */
		...
		function _setBindings (selector){
			...
			var options = viewers[selector].options, draggingTimeInterval;

			// Hover binding for the container
			$(selector).hover(function (){

				// Shows the container and its elements with a fade
				if (!$(selector + ' .chameleon-images-current').is(':focus'))
			...
			$(selector).keydown(function(event) {
				...
				if ($(selector + ' .chameleon-images-current').is(':focus') && event.which == 13){
					...
					_showImage(selector, options.currentImage);
					$(selector).focus();
			...
			$(selector).on('mousewheel', function(event) {
				...
				if ((options.currentZoom > options.minZoom && event.deltaY < 0) ||
					(options.currentZoom < options.maxZoom && event.deltaY > 0)){

					// New zoom value
					var newZoom = viewers[selector].options.currentZoom + viewers[selector].options.zoomStep*event.deltaY;
			...
			$(selector).on('mousedown', function(event){
				...
				viewers[selector].options.cursorDown.pixelX = event.pageX - $(this).offset().left;
				viewers[selector].options.cursorDown.pixelY = event.pageY - $(this).offset().top;

				// Mark the selector as dragagable
				viewers[selector].options.dragging = true;
				...
				if (options.dragging &&
					...
					var dimX = (event.pageX - $(this).offset().left) - viewers[selector].options.cursorDown.pixelX,
						dimY = (event.pageY - $(this).offset().top) - viewers[selector].options.cursorDown.pixelY;
				...
				viewers[selector].options.dragging = false;
	...
	// TODO: external allowed functions
